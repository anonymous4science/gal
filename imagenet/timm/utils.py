...
def unwrap_model(model):
    if isinstance(model, ModelEma) or isinstance(model, ModelEma_gal):
...
class CheckpointSaver:
    ...
    def save_checkpoint_gal(self, model, regressor, optimizer, args, epoch, model_ema=None, regressor_ema=None, metric=None, use_amp=False):
        assert epoch >= 0
        worst_file = self.checkpoint_files[-1] if self.checkpoint_files else None
        if (len(self.checkpoint_files) < self.max_history
                or metric is None or self.cmp(metric, worst_file[1])):
            ...
            filename = '-'.join([self.save_prefix, str(epoch)]) + self.extension
            save_path = os.path.join(self.checkpoint_dir, filename)
            self._save_gal(save_path, model, regressor, optimizer, args, epoch, model_ema, regressor_ema, metric, use_amp)
            self.checkpoint_files.append((save_path, metric))
            self.checkpoint_files = sorted(
                self.checkpoint_files, key=lambda x: x[1],
                reverse=not self.decreasing)  # sort in descending order if a lower metric is not better
    ...
    def _save_gal(self, save_path, model, regressor, optimizer, args, epoch, model_ema=None, regressor_ema=None, metric=None, use_amp=False):
        save_state = {
            'epoch': epoch,
            'arch': args.model,
            'state_dict': get_state_dict(model),
            'gal_state_dict': get_state_dict(regressor),
            'optimizer': optimizer.state_dict(),
            'args': args,
            'version': 2,  # version < 2 increments epoch before save
        }
        ...
        if model_ema is not None:
            save_state['state_dict_ema'] = get_state_dict(model_ema)
            save_state['gal_state_dict_ema'] = get_state_dict(regressor_ema)
        if metric is not None:
            save_state['metric'] = metric
        torch.save(save_state, save_path)
...
class ModelEma_gal:
    ...
    def _load_checkpoint(self, checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        assert isinstance(checkpoint, dict)
        if 'gal_state_dict_ema' in checkpoint:
            new_state_dict = OrderedDict()
            for k, v in checkpoint['gal_state_dict_ema'].items():
                # ema model may have been wrapped by DataParallel, and need module prefix
                if self.ema_has_module:
                    name = 'module.' + k if not k.startswith('module') else k
                else:
                ...
                new_state_dict[name] = v
            self.ema.load_state_dict(new_state_dict)
            logging.info("Loaded gal_state_dict_ema")
        else:
            logging.warning("Failed to find gal_state_dict_ema, starting from loaded model weights")
