...
def add_weight_decay_lr(model, weight_decay=1e-5, lr=1e-3, skip_list=()):
    decay = []
    no_decay = []
    for name, param in model.named_parameters():
        if not param.requires_grad:
...
def create_optimizer_gal(args, model, regressor, filter_bias_and_bn=True):
    opt_lower = args.opt.lower()
    weight_decay = args.weight_decay
    if 'adamw' in opt_lower or 'radam' in opt_lower:
        # Compensate for the way current AdamW and RAdam optimizers apply LR to the weight-decay
    ...
    if weight_decay and filter_bias_and_bn:
        parameters = add_weight_decay(model, weight_decay)
        parameters_lr = add_weight_decay_lr(regressor, weight_decay, args.gal_lr)
        parameters = list(parameters) + list(parameters_lr)
        weight_decay = 0.
    else:
        parameters = model.parameters()
        parameters_lr = regressor.parameters()
        parameters = list(parameters) + list(parameters_lr)
